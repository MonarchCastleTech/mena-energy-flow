# -*- coding: utf-8 -*-
"""MENA Energy Flow Tracker - Data Extraction Pipeline"""
import os
import json
import yaml
import requests
...
def generate_demo_data(config):
    ...
    return {
        "meta": {
            ...
            "sources": ["AIS Shipping", "EIA", "Google News RSS", "Global Energy Monitor"],
            "version": "1.0.0"
        },
        "stats": [
            {"label": "Tankers Active", "value": "892", "delta": "+5%"},
            {"label": "Chokepoints", "value": "6", "delta": "+2"},
            {"label": "bbl/day Tracked", "value": "4.2M", "delta": "-3%"},
            {"label": "Price Correlations", "value": "0.73", "delta": "stable"},
        ],
        "entities": [
        ...
        "events": [
            {"id": i, "title": f"Event {i}", "severity": ["low", "medium", "high"][i % 3], "timestamp": datetime.now(timezone.utc).isoformat(), "source": "AIS Shipping"}
            for i in range(1, 16)
        ],
        "timeseries": [
